...
const STORAGE_KEYS = {
  ENERGY_QUEUE: 'gcip_energy_queue',
  LEADERBOARD: 'gcip_leaderboard',
  HISTORY: 'gcip_history',
  USER_TOTAL_KWH: 'gcip_user_total_kwh',
  USER_ENERGY_LOGS: 'gcip_user_energy_logs',
  USER_QUESTS: 'gcip_user_quests'
};

const addToEnergyQueue = (kwh: number) => {
  try {
    const queue = JSON.parse(localStorage.getItem(STORAGE_KEYS.ENERGY_QUEUE) || '[]');
    queue.push(kwh);
    localStorage.setItem(STORAGE_KEYS.ENERGY_QUEUE, JSON.stringify(queue));
  } catch (e) { console.error("Storage error", e); }
};

const consumeEnergyQueue = (): number => {
  try {
    const queue = JSON.parse(localStorage.getItem(STORAGE_KEYS.ENERGY_QUEUE) || '[]');
    if (queue.length === 0) return 0;
    const total = queue.reduce((a: number, b: number) => a + b, 0);
    localStorage.setItem(STORAGE_KEYS.ENERGY_QUEUE, '[]');
    return total;
  } catch (e) { return 0; }
};
...
const INITIAL_USER_ENERGY_LOGS: UserEnergyLog[] = [
  { date: 'Mon', kwh: 12.5 },
  { date: 'Tue', kwh: 15.2 },
  { date: 'Wed', kwh: 8.4 },
  { date: 'Thu', kwh: 18.1 },
...
const INITIAL_QUESTS: Omit<Quest, 'icon'>[] = [
    { id: 1, title: 'Recycling Rookie', description: 'Log 5kg of waste today', target: 5, current: 0, reward: 100, unit: 'kg', type: 'waste', completed: false },
    { id: 2, title: 'Energy Watchdog', description: 'Submit 2 energy reports', target: 2, current: 0, reward: 150, unit: 'reports', type: 'energy', completed: false },
];
...
const LoginScreen = ({ onLogin }: { onLogin: (role: View) => void }) => (
...
const AdminDashboard = ({ onLogout }: { onLogout: () => void }) => {
  // State moved inside to prevent Main App re-renders
  ...
  // Simulated IoT Data Stream (Local to AdminDashboard)
  useEffect(() => {
    const interval = setInterval(() => {
      setEnergyData((prev) => {
        ...
        // Check for user input from storage (The "Local Process")
        const userAddedLoad = consumeEnergyQueue();
        
        // Base random load + User Input
        const baseLoad = 100 + Math.random() * 100 + (Math.random() > 0.8 ? 150 : 0);
        const finalLoad = baseLoad + userAddedLoad;
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={energyData}>
                ...
                <Area 
                  ...
                  fillOpacity={1} 
                  fill="url(#colorKwh)"
                  isAnimationActive={false} // Disable animation on update to prevent jump
                />
              </AreaChart>
            </ResponsiveContainer>
          </div>
...
const UserApp = ({ leaderboard, pointsHistory, onEarnPoints, onLogout }: any) => {
  // --- State for Forms ---
  ...
  // LOAD FROM STORAGE
  const [totalKwh, setTotalKwh] = useState(() => loadState(STORAGE_KEYS.USER_TOTAL_KWH, 324.5));
  const [energyLogs, setEnergyLogs] = useState<UserEnergyLog[]>(() => 
      loadState(STORAGE_KEYS.USER_ENERGY_LOGS, INITIAL_USER_ENERGY_LOGS)
  );
  const [quests, setQuests] = useState<Quest[]>(() => {
      const loaded: Omit<Quest, 'icon'>[] = loadState(STORAGE_KEYS.USER_QUESTS, INITIAL_QUESTS);
      // Re-attach icons
      return loaded.map(q => ({
          ...q,
          icon: q.type === 'waste' ? Recycle : Zap
      }));
  });
  ...
  const handleShowSuccess = (points: number, message: string) => {
  ...
  const updateQuests = (type: 'waste' | 'energy', amount: number) => {
      setQuests(prev => {
          const updated = prev.map(q => {
              if (q.type === type && !q.completed) {
                  const newCurrent = q.current + amount;
                  if (newCurrent >= q.target) {
                      onEarnPoints(q.reward, `Quest Completed: ${q.title}`);
          ...
          // Save stripped version to avoid icon serialization issues
          const toSave = updated.map(({ icon, ...rest }) => rest);
          saveState(STORAGE_KEYS.USER_QUESTS, toSave);
          return updated;
      });
  };
  ...
  const handleSubmitWaste = (e: React.FormEvent) => {
    ...
    const action = `Recycled ${wasteType.charAt(0).toUpperCase() + wasteType.slice(1)}`;
    onEarnPoints(points, action);
    handleShowSuccess(points, action);
    
    // Update Quests
    updateQuests('waste', weight);
  ...
  const handleSubmitEnergy = (e: React.FormEvent) => {
    ...
    setTotalKwh(prev => {
       const newVal = prev + kwh;
       saveState(STORAGE_KEYS.USER_TOTAL_KWH, newVal);
       return newVal;
    });
    
    // Update Energy Logs Chart
    const newLog = {
        date: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
        kwh: kwh
    };
    const updatedLogs = [...energyLogs, newLog].slice(-7); // Keep last 7 to prevent overcrowding
    setEnergyLogs(updatedLogs);
    saveState(STORAGE_KEYS.USER_ENERGY_LOGS, updatedLogs);

    // Affect Admin Graph
    addToEnergyQueue(kwh);
    
    // Update Quests
    updateQuests('energy', 1); // 1 report submitted

    const action = `Energy Report (${building})`;
    onEarnPoints(50, action);
    handleShowSuccess(50, action);
  ...
  const userPoints = leaderboard.find((u: LeaderboardEntry) => u.isUser)?.points || 0;
      ...
      <div className="bg-emerald-600 text-white p-6 rounded-b-[2rem] shadow-lg relative overflow-hidden">
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-6">
            ...
            <button onClick={onLogout} className="text-emerald-100 hover:bg-emerald-700/50 p-2 rounded-full transition-colors">
          ...
          <div className="flex justify-between items-end mb-6">
            ...
            <div className="bg-emerald-500/50 px-3 py-1 rounded-full text-xs font-semibold backdrop-blur-md border border-emerald-400/30">
              Rank #{leaderboard.findIndex((u: LeaderboardEntry) => u.isUser) + 1}
            </div>
          </div>
      ...
      <div className="px-4 -mt-6 relative z-20 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
           <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] bg-red-50 text-red-600 px-2 py-1 rounded-full font-bold">Resets in 4h</span>
           </div>
           
           <div className="space-y-3">
              {quests.map((quest) => (
                 <div key={quest.id} className={`relative p-4 rounded-xl border transition-all ${quest.completed ? 'bg-emerald-50 border-emerald-100' : 'bg-slate-50 border-slate-100'}`}>
                    <div className="flex justify-between items-start mb-2 relative z-10">
                       <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg ${quest.completed ? 'bg-emerald-100 text-emerald-600' : 'bg-white text-slate-400'}`}>
                             <quest.icon className="w-5 h-5" />
                          </div>
                          <div>
                             <h4 className={`text-sm font-bold ${quest.completed ? 'text-emerald-900' : 'text-slate-800'}`}>{quest.title}</h4>
                             <p className="text-[10px] text-slate-500">{quest.description}</p>
                          </div>
                       </div>
                       {quest.completed ? (
                          <span className="flex items-center gap-1 text-xs font-bold text-emerald-600 bg-white/50 px-2 py-1 rounded-lg">
                             <Check className="w-3 h-3" /> Done
                          </span>
                       ) : (
                          <div className="flex items-center gap-1 bg-amber-100 text-amber-700 px-2 py-1 rounded-lg">
                             <Gift className="w-3 h-3" />
                             <span className="text-[10px] font-bold">+{quest.reward} pts</span>
                          </div>
                       )}
                    </div>
                    ...
                    <div className="relative z-10 mt-2">
                       <div className="flex justify-between text-[10px] font-medium text-slate-500 mb-1">
                          <span>Progress</span>
                          <span>{Math.min(quest.current, quest.target)} / {quest.target} {quest.unit}</span>
                       </div>
            ...
            <div className="mx-5 mb-5 p-4 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-600">
                  {successMessage ? successMessage : `You earned +${earnedPointsDisplay} points.`}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 mb-6">
           <div className="flex items-center gap-2 mb-6">
              ...
              <div>
                 <h3 className="font-bold text-slate-800 leading-tight">My Energy Footprint</h3>
                 <p className="text-[10px] text-slate-400">Recent submissions (kWh)</p>
              </div>
           </div>
           
           <div className="h-[200px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                 <BarChart data={energyLogs} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="space-y-3">
            {pointsHistory.length === 0 ? (
              ...
              <>
                {visibleHistory.map((entry: HistoryEntry) => (
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="space-y-3">
            {sortedLeaderboard.map((user: LeaderboardEntry, idx: number) => (
              <div 
                key={user.id} 
...
// --- Main App Component (Stable Shell) ---
const App = () => {
  const [currentView, setCurrentView] = useState<View>('login');
  
  // Shared State
  const [leaderboard, setLeaderboard] = useState<LeaderboardEntry[]>(() => 
    loadState(STORAGE_KEYS.LEADERBOARD, INITIAL_LEADERBOARD)
  );
  const [pointsHistory, setPointsHistory] = useState<HistoryEntry[]>(() => 
    loadState(STORAGE_KEYS.HISTORY, INITIAL_HISTORY)
  );

  const handlePointsEarned = (points: number, action: string) => {
    // Update Leaderboard & Save
    const updatedLeaderboard = leaderboard.map(entry => 
      entry.isUser ? { ...entry, points: entry.points + points } : entry
    ).sort((a, b) => b.points - a.points);
    setLeaderboard(updatedLeaderboard);
    saveState(STORAGE_KEYS.LEADERBOARD, updatedLeaderboard);
    ...
    saveState(STORAGE_KEYS.HISTORY, updatedHistory);
  };

  return (
    <>
      {currentView === 'login' && <LoginScreen onLogin={setCurrentView} />}
      {currentView === 'admin' && <AdminDashboard onLogout={() => setCurrentView('login')} />}
      {currentView === 'user' && (
        <UserApp 
           onLogout={() => setCurrentView('login')} 
           leaderboard={leaderboard}
           pointsHistory={pointsHistory}
           onEarnPoints={handlePointsEarned}
        />
      )}
    </>
  );
